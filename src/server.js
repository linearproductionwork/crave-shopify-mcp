...
function taxSuggestionForProduct(product) {
  const text = [
    product.title,
    product.productType,
    product.vendor,
  ...
  const likelyTaxable = [
    "tool", "mold", "pan", "packaging", "box", "equipment", "utensil",
    "cutter", "spatula", "merch", "gift card", "candy", "confection"
  ];

  const likelyExempt = [
    "spread", "filling", "glaze", "cocoa", "sugar", "glucose", "improver",
    "paste", "ganache", "pastry shell", "bakery", "baking", "ingredient",
    "vela", "supernortemul", "nortemul", "pistachio", "nocciola"
  ];

  if (likelyTaxable.some((term) => text.includes(term))) {
    return "REVIEW/TAXABLE: parece herramienta, packaging, candy o producto no alimentario.";
  }

  if (likelyExempt.some((term) => text.includes(term))) {
    return "LIKELY_EXEMPT_FL: parece ingrediente de reposteria/grocery empaquetado.";
  }

  return "REVIEW: faltan datos para clasificar con confianza.";
}

const server = new McpServer({
  name: "crave-shopify-mcp",
  version: "1.0.0"
});

server.tool(
  "list_products",
  "List Shopify products with product type, tags, vendor, status, and taxability signals.",
  {
  ...
  async ({ first }) => {
    ...
    const products = data.products.nodes.map((product) => ({
      ...product,
      floridaTaxSuggestion: taxSuggestionForProduct(product)
    }));
...
app.listen(Number(PORT), () => {
  console.log(`Crave Shopify MCP listening on port ${PORT}`);
});
